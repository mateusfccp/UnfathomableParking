...
public class ListSelectionScene<T> : IScene
{
    ...
    public ListSelectionScene(List<T> options, uint initialSelectedIndex = 0, Func<T, string>? formatter = null,
    ...
    public void Draw(Engine.Canvas canvas)
    {
        ...
        var height = (uint)Math.Min(options.Count + 2, canvas.Height);
    ...
    public void OnKeyPressed(ConsoleKeyInfo keyInfo)
    {
        ...
        {
            ConsoleKey.UpArrow => (uint)((_selectedIndex - 1 + options.Count) % options.Count),
            ConsoleKey.DownArrow => (uint)((_selectedIndex + 1) % options.Count),
            _ => _selectedIndex
        };

        if (keyInfo.Key == ConsoleKey.Enter)
        {
            _onSelect(options[(int)_selectedIndex]);
        }
    }
}
